(ns ^:integration riverford.durable-ref.scheme.dynamodb.amazonica-integration-test
  (:require [clojure.test :refer :all]
            [riverford.durable-ref.scheme.dynamodb.amazonica]
            [riverford.durable-ref.core :as dref])
  (:import (java.net URI)
           (java.util UUID)))

(def uri-base
  (or (System/getenv "DURABLE_REF_TEST_DYNAMODB_PREFIX")
      (System/getProperty "durable.ref.test.dynamodb.prefix")
      (.println System/err "Warning: DURABLE_REF_TEST_DYNAMODB_PREFIX not set, skipping some tests.")))
...
(deftest test-concurrent-swaps
  (when uri-base
    (let [uri (URI. (str "atomic:" uri-base "/test-" (UUID/randomUUID) ".edn"))
